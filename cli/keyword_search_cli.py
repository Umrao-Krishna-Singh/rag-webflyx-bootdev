...
def main() -> None:
    ...
    match args.command:
        ...
        case "bm25tf":
            movie_idx = InvertedIndex()
            movie_idx.load()
            bm25tf = movie_idx.get_bm25_tf(args.doc_id, args.term, args.k1, args.b)
            print(
        ...
        case "bm25search":
            print(f"Searching for: {args.query}")
            movie_idx = InvertedIndex()
            movies = movie_idx.bm25_search(args.query)
            for i, movie in enumerate(movies):
                match, score = movie
                print(f"{i+1}. ({match['id']}) {match['title']} - Score: {score:.2f}")
